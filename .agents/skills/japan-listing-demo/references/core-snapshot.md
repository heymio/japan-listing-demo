...
The upstream repository remains the maintenance source for generic methods. This repository publishes a tested snapshot plus Japan distribution patches so Japan-team users receive one repository, one ZIP, and one Skill invocation.

When updating the snapshot:

1. review the upstream changelog and commit;
2. update only the bundled core files affected;
3. preserve or deliberately revise Japan distribution patches and channel profiles;
4. run all core, Japan, channel, cross-category, frontend-fidelity, packaging, and leakage checks;
